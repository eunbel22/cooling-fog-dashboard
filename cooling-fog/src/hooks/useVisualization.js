            ...
            <div
              ...
              onMouseDown={(e) => {
                ...
                if (isManualMode) {
                  e.preventDefault();
                  e.stopPropagation();
                  const webGridIndex = index;
                  const gridName = grid.name;
                  const currentGridName = patrolCurrentGrid !== null ? GRID_POSITIONS[patrolCurrentGrid]?.name : '대기중';
                  console.log(`🖱️ [클릭 상세정보]`);
                  console.log(`   웹 인덱스: ${webGridIndex} (0-based)`);
                  console.log(`   그리드명: ${gridName}`);
                  console.log(`   현재 순찰 구역: ${currentGridName}`);
                  console.log(`   → 라즈베리파이로 전송할 값: ${webGridIndex}`);
